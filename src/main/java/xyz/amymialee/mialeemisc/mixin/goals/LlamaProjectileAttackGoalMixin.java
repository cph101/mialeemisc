...
public class LlamaProjectileAttackGoalMixin<T extends HostileEntity & RangedAttackMob> extends ProjectileAttackGoalMixin<T> {
    @Override
    protected void mialeeMisc$universalRangedAttack(RangedAttackMob owner, LivingEntity target, float pullProgress, Operation<Void> original) {
        if (owner instanceof LivingEntity living && IUniversalRangedWeapon.canRangedAttack(living, target, pullProgress)) {
